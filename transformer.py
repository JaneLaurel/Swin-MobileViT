...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=8, shift_size=0,
        ...
        self.mlp = nn.Sequential(
            nn.LayerNorm(dim),
            nn.Linear(in_features=dim, out_features=mlp_hidden_dim, bias=True),
            nn.SiLU(),
            nn.Dropout(p=drop),
            nn.Linear(in_features=mlp_hidden_dim, out_features=dim, bias=True),
            nn.Dropout(p=drop)
        )
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, ffn_dim, ffn_dropout, window_size,
                 mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False):
        super().__init__()
        self.dim = dim
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        # self.window_size = window_size
        #更改为W-MSA\SW-MSA,，由参数i即循环当前的层数单双数决定使用哪一个
        # 单数使用W-MSA，双数使用SW-MSA
        ...
        # attn_unit = WindowAttention(
        #     dim=embed_dim, window_size=(self.window_size, self.window_size), num_heads=num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.
        # )
